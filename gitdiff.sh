...
function fetch_commit_by_id()
{
    ...
    mkdir -p "$TARGET_PATH"
    [ $GENERATE_DIFF = 1 ] && git diff "$BEFORE_COMMIT" "$AFTER_COMMIT" > "$TARGET_PATH/commit.diff"
...
{
    mkdir -p "$TARGET_PATH"
    [ $GENERATE_DIFF = 1 ] && git diff > "$TARGET_PATH/current.diff"
...
function fetch_branch_by_id()
{
    ...
    mkdir -p "$TARGET_PATH"
    [ $GENERATE_DIFF = 1 ] && git diff "$1" > "$TARGET_PATH/branch.diff"
